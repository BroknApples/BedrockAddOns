...
const CustomCropGrowthBlockComponent = {
    onRandomTick({ block }) {
        ...
        const growth = block.permutation.getState("cnc:growth");
        block.setPermutation(block.permutation.withState("cnc:growth", growth + 1));
    },
    onPlayerInteract({ block, dimension, player }) {
        ...
        if (player.getGameMode() === GameMode.creative) {
            // Grow crop fully
            block.setPermutation(block.permutation.withState("cnc:growth", 7));
        } else {
            let growth = block.permutation.getState("cnc:growth");

            // Add random amount of growth
            growth += randomInt(1, maxGrowth - growth);
            block.setPermutation(block.permutation.withState("cnc:growth", growth));
